...
- `multiple_ask_jobs` and per-item `multiple_ask_job_items` are durable domain records separate from `job_queue.status`. Railway canonically validates bounded private bytes before committing the existing batch quota. Pasted text and embedded PDF text use deterministic extraction; scanned pages use server-only Gemini Vision structured extraction, never textbooks or answer generation. Item modes are `short|long|mcq|not_clear`; a mixed batch never persists `mixed` for an item.
