...
- **Status:** The public Ask path, real Supabase/Upstash/DeepSeek integration, Railway/Vercel deployment, approved/grounded/general staging, and CI exit checks pass. Module 4 remains open only for the owner's public WhatsApp support setting.
...
- **Remaining exit work:** `academy_settings/default` does not exist in Firestore, so the optional limit-exceeded WhatsApp action is safely hidden. The owner must provide the public WhatsApp number and default message before Module 4 is marked complete.
