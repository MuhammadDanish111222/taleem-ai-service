...
async def test_concurrent_lease_with_independent_connections():
    ...
    try:
        ...
        job = await repo1.create_job(
            "concurrent_job", {"data": 123}, idempotency_key="idemp_concurrent_1"
        )
        ...
        assert len(leased_results) == 1, (
            "Exactly one worker must acquire lease under FOR UPDATE SKIP LOCKED"
        )
        ...
        # Cleanup test job
        await conn1.execute(
            "DELETE FROM job_queue WHERE idempotency_key = 'idemp_concurrent_1';"
        )
        await conn1.close()
        await conn2.close()
...
async def test_lock_ownership_enforcement(db_conn):
    ...
    await repo.create_job("ownership_test_job", {"data": 1})
...
async def test_worker_crash_stale_recovery_and_idempotency(db_conn):
    ...
    job = await service.enqueue_job(
        "crash_test_job", {"target_id": "target_res_123"}, idempotency_key=idemp_key
    )
    job_id = str(job["id"])
    ...
    await db_conn.execute(
        "UPDATE job_queue SET next_retry_at = NOW() WHERE id = $1::uuid;", job_id
    )
    ...
    assert output_count == 1, (
        "Side-effect must be strictly idempotent with exactly 1 output record"
    )
...
async def test_unsupported_job_type_handling(db_conn):
    ...
    worker = Worker(
        worker_id="test_worker_unsupported", supported_types=["unsupported_type_xyz"]
    )
    service = JobQueueService(db_conn)
    ...
    pool_mock = type(
        ...
        (),
        {
            "acquire": lambda self_inner: type(
                "AcquireCtx",
                (),
                {
...
async def test_job_retry_backoff(db_conn):
    ...
    fail_ok = await service.fail_job(
        job_id, "worker_1", "TRANSIENT_ERR", "Temporary failure", retry_delay_seconds=5
    )
...
async def test_job_attempt_exhaustion(db_conn):
    ...
    fail_ok = await service.fail_job(
        job_id, "worker_1", "PERMANENT_ERR", "Permanent failure", retry_delay_seconds=5
    )
    assert fail_ok is True
...
async def test_real_worker_process_crash_and_recovery():
    ...
    try:
        ...
        job = await service.enqueue_job(
            "os_crash_job", {"target": "res_os_crash"}, idempotency_key=idemp_key
        )
        job_id = str(job["id"])
...
"""
        ...
        try:
            ...
            await conn_test.execute(
                "UPDATE job_queue SET next_retry_at = NOW() WHERE id = $1::uuid;",
                job_id,
            )
        ...
        await conn_test.execute(
            "DELETE FROM job_queue WHERE idempotency_key = 'idemp_os_proc_crash_999';"
        )
