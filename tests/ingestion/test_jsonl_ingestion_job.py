...
def mock_firestore_hierarchy_check():
    ...
    mock_db = MagicMock()
    mock_app = patch(
        "app.workers.handlers.jsonl_ingest.get_firebase_app", return_value=object()
    )
    mock_client = patch("firebase_admin.firestore.client", return_value=mock_db)
    mock_check = patch(
        "app.services.ingestion.jsonl_chunks.check_firestore_hierarchy",
        return_value=True,
    )
    mock_counter = patch(
        "app.workers.handlers.jsonl_ingest.get_token_counter",
        return_value=FakeTokenCounter(),
    )
    with mock_app, mock_client, mock_check, mock_counter:
        yield
...
async def test_chapter_reupload_atomic_replace(conn):
    ...
    job1 = {
        "id": str(uuid.uuid4()),
        "job_type": "jsonl_ingest",
        "payload": {"jsonl_content": raw_jsonl_1},
    }
    res1 = await handle_jsonl_ingest(job1, conn)
    assert res1["chunks_ingested"] == 3

    corpus_version_id = res1["corpus_version_id"]
    cv1 = await conn.fetchrow(
        "SELECT expected_chunk_count FROM rag_corpus_versions WHERE id = $1::uuid;",
        corpus_version_id,
    )
    ...
    chunks = await conn.fetch(
        "SELECT * FROM rag_chunks WHERE corpus_version_id = $1::uuid;",
        corpus_version_id,
    )
    assert len(chunks) == 1
...
async def test_multi_chapter_corpus_accumulation(conn):
    ...
    jsonl_ch1 = (
        ...
        f'"topic_no":"1.1","topic_title":"Ch1","chunk_order":0,"content_type":"explanation","chunk_text":"Ch1 text","expected_questions":[]}}'
    )
    jsonl_ch2 = (
        f'{{"board_id":"{board_id}","class_id":"{class_id}","subject_id":"{subject_id}","chapter_id":"{ch2_id}",'
        f'"topic_no":"2.1","topic_title":"Ch2","chunk_order":0,"content_type":"explanation","chunk_text":"Ch2 text","expected_questions":[]}}'
    )

    job1 = {
        "id": str(uuid.uuid4()),
        "job_type": "jsonl_ingest",
        "payload": {"jsonl_content": jsonl_ch1},
    }
    res1 = await handle_jsonl_ingest(job1, conn)
    ...
    job2 = {
        "id": str(uuid.uuid4()),
        "job_type": "jsonl_ingest",
        "payload": {"jsonl_content": jsonl_ch2},
    }
    res2 = await handle_jsonl_ingest(job2, conn)
...
async def test_non_building_status_lock_rejection(conn):
    ...
    cv = await repo.get_or_create_building_corpus_version(
...
async def test_first_ingestion_concurrent_corpus_creation_race():
    ...
    cv1 = await repo1.get_or_create_building_corpus_version(
        board_id, class_id, subject_id
    )
    ...
    async def conn2_task():
        ...
        await tx2.start()
        try:
            res = await repo2.get_or_create_building_corpus_version(
                board_id, class_id, subject_id
            )
            await tx2.commit()
            return res
    ...
    total_versions = await conn1.fetchval(
        "SELECT COUNT(*) FROM rag_corpus_versions WHERE corpus_id = $1::uuid;",
        cv1["corpus_id"],
    )
...
async def test_jsonl_ingest_idempotency_key_replay(conn):
    ...
    job1 = await service.enqueue_job(
        "jsonl_ingest", payload, idempotency_key=idempotency_key
    )
    leased_job = await service.lease_next_job("worker-1", ["jsonl_ingest"])
    await handle_jsonl_ingest(leased_job, conn)
    ...
    job2 = await service.enqueue_job(
        "jsonl_ingest", payload, idempotency_key=idempotency_key
    )
...
async def test_jsonl_ingest_fails_when_firestore_unavailable(conn):
    ...
    job = {
        "id": str(uuid.uuid4()),
        "job_type": "jsonl_ingest",
        "payload": {"jsonl_content": raw_jsonl},
    }

    with patch("app.workers.handlers.jsonl_ingest.get_firebase_app", return_value=None):
        with pytest.raises(
            RuntimeError,
            match="Firestore client unavailable for mandatory catalogue hierarchy verification",
        ):
...
async def test_jsonl_ingest_handler_direct_reexecution_idempotency(conn):
    ...
    d_count_1 = await conn.fetchval(
        "SELECT COUNT(*) FROM rag_document_versions WHERE id = $1::uuid;", doc_ver_id1
    )
    c_count_1 = await conn.fetchval(
        "SELECT COUNT(*) FROM rag_chunks WHERE document_version_id = $1::uuid;",
        doc_ver_id1,
    )
    ...
    d_count_2 = await conn.fetchval(
        "SELECT COUNT(*) FROM rag_document_versions WHERE id = $1::uuid;", doc_ver_id1
    )
    c_count_2 = await conn.fetchval(
        "SELECT COUNT(*) FROM rag_chunks WHERE document_version_id = $1::uuid;",
        doc_ver_id1,
    )
...
@pytest.mark.asyncio
async def test_expected_question_rows_are_individual_and_reingestion_does_not_duplicate_them(
    conn,
):
    board_id = f"b_questions_{uuid.uuid4().hex[:6]}"
    class_id = f"c_questions_{uuid.uuid4().hex[:6]}"
    subject_id = f"s_questions_{uuid.uuid4().hex[:6]}"
    chapter_id = f"ch_questions_{uuid.uuid4().hex[:6]}"
    raw_jsonl = (
        f'{{"board_id":"{board_id}","class_id":"{class_id}","subject_id":"{subject_id}","chapter_id":"{chapter_id}",'
        f'"topic_no":"1","topic_title":"Questions","chunk_order":0,"content_type":"explanation",'
        f'"chunk_text":"Content","expected_questions":["Question one?","Question two?"]}}'
    )
    job = {
        "id": str(uuid.uuid4()),
        "job_type": "jsonl_ingest",
    ...
    first = await handle_jsonl_ingest(job, conn)
    second = await handle_jsonl_ingest(job, conn)
    assert first["document_version_id"] == second["document_version_id"]

    questions = await conn.fetch(
        """
        SELECT question_text, embedding FROM chunk_expected_questions
        WHERE chunk_id IN (SELECT id FROM rag_chunks WHERE document_version_id = $1::uuid)
    ...
    assert all(row["embedding"] is None for row in questions)
