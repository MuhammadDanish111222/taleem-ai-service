...
async def test_job_repository_lifecycle(db_conn):
    ...
    job = await repo.create_job(
        "ingest_pipeline", {"resource_id": "res_001"}, idempotency_key="idemp_001"
    )
...
async def test_rag_repository_crud_vector_and_lexical(db_conn):
    ...
    vector_results = await repo.search_chunks_vector(
        str(cv["id"]), sample_vector, top_k=5
    )
    ...
    lexical_results = await repo.search_chunks_lexical(
        str(cv["id"]), "measurement physics", top_k=5
    )
