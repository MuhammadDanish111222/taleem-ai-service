...
async def conn():
    try:
        connection = await asyncpg.connect(
            os.getenv("TEST_DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/taleem_dev")
        )
...
async def add_question(
    ...
    *,
    ...
    revision_id = await conn.fetchval(
        ...
        question_id, *SCOPE, chapter, mode, difficulty, marks,
        f"Question {uuid.uuid4()}", "a" * 64, status, superseded,
    )
...
def section(
    key: str, *, select_count: int, attempt_count: int | None = None,
    chapters: dict[str, int] | None = None, difficulties: dict[str, int] | None = None,
    mode: str = "short", marks: int = 2,
) -> dict:
    return {"key": key, "title": key, "type": mode, "select_count": select_count,
            "attempt_count": attempt_count or select_count, "marks_each": marks,
            "chapter_distribution": chapters or {}, "difficulty_distribution": difficulties or {}}


async def select(conn: asyncpg.Connection, value: dict, seed: str = "seed") -> dict:
    raw = await conn.fetchval("SELECT taleem_select_questions($1,$2,$3,$4::jsonb,$5)", *SCOPE, json.dumps(value), seed)
...
async def test_selector_is_exact_deterministic_and_excludes_unusable_revisions(conn):
    ...
    value = spec([section("B", select_count=4, attempt_count=2, chapters={"chapter-1": 2, "chapter-2": 2}, difficulties={"easy": 2, "hard": 2})])
    first, second = await select(conn, value, "same-seed"), await select(conn, value, "same-seed")
    assert first["satisfiable"] is True
    assert [item["revision_id"] for item in first["selected"]] == [item["revision_id"] for item in second["selected"]]
...
async def test_selector_rejects_joint_failure_and_never_duplicates(conn):
    ...
    await add_question(conn, chapter="chapter-2", difficulty="hard")
    result = await select(conn, spec([section("B", select_count=2, chapters={"chapter-1": 1, "chapter-2": 1}, difficulties={"easy": 2})]))
    assert result["satisfiable"] is False
    assert result["reason"] == "EXACT_ALLOCATION_UNSATISFIED"
    await add_question(conn, chapter="chapter-3", difficulty="medium")
    result = await select(conn, spec([section("B1", select_count=1), section("B2", select_count=1)]))
...
async def test_selector_seed_changes_the_valid_choice_when_alternatives_exist(conn):
    ...
    choices = {
        tuple(item["question_id"] for item in (await select(conn, value, f"seed-{index}"))["selected"])
...
async def test_blueprint_validation_and_activation_are_database_enforced(conn):
    ...
    with pytest.raises(asyncpg.RaiseError):
        await conn.execute("""INSERT INTO board_paper_blueprints(board_id,class_id,subject_id,name,config,is_active,created_by,updated_by)
                              VALUES($1,$2,$3,'bad',$4::jsonb,FALSE,'test','test')""", *SCOPE, json.dumps(spec([section("B", select_count=1, attempt_count=2)])))
    await conn.execute("""INSERT INTO board_paper_blueprints(board_id,class_id,subject_id,name,config,is_active,created_by,updated_by)
                          VALUES($1,$2,$3,'valid',$4::jsonb,TRUE,'test','test')""", *SCOPE, json.dumps(valid))
    with pytest.raises(asyncpg.UniqueViolationError):
        await conn.execute("""INSERT INTO board_paper_blueprints(board_id,class_id,subject_id,name,config,is_active,created_by,updated_by)
                              VALUES($1,$2,$3,'second',$4::jsonb,TRUE,'test','test')""", *SCOPE, json.dumps(valid))
    with pytest.raises(asyncpg.RaiseError):
        await conn.execute(
            ...
            SCOPE[0], "module6-empty-class", SCOPE[2], json.dumps(valid),
        )
