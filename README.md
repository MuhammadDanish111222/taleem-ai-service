...
- **Module 5 Run 2 (still dark)**: Railway-only local Tesseract OCR, durable temporary normalized source text, deterministic ordered question extraction (up to 60 items), correction/resume, and polling APIs are present. Objective decimal numbering/MCQs and subjective Roman or lettered subparts remain individually ordered. Set Railway's `RAILPACK_DEPLOY_APT_PACKAGES="tesseract-ocr tesseract-ocr-eng"` so the final image contains the local binary and English data. Keep `MULTIPLE_ASK_RUN1_ENABLED=false`; no answer pipeline or student page is included.
