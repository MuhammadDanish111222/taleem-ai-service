...
Deno.serve(async (request) => {
  ...
  if (!isObject(body) || "uid" in body || (body.mode !== "board" && body.mode !== "custom")
      || ![body.board_id, body.class_id, body.subject_id, body.seed].every((item) => typeof item === "string" && scope.test(item))) return fail("INVALID_REQUEST", 400);
  if (body.mode === "board" ? ("spec" in body) : !isObject(body.spec)) return fail("INVALID_REQUEST", 400);
  if (body.seed !== claims.request_id) return fail("INVALID_REQUEST", 400);
