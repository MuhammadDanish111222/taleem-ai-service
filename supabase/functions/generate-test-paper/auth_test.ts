...
async function token(overrides: { uid?: string; feature?: string; audience?: string; issuer?: string } = {}) {
  return new SignJWT({ uid: overrides.uid ?? "student", feature: overrides.feature ?? "test_generator", request_id: "request" })
    ...
    .setIssuer(overrides.issuer ?? "taleem-web")
    .setAudience(overrides.audience ?? "taleem-test-generator")
    .setIssuedAt()
    .setJti("jti")
...
for (const [name, overrides] of [
  ["wrong audience", { audience: "taleem-ai-service" }],
  ["wrong issuer", { issuer: "bad" }],
  ["wrong feature", { feature: "ask" }],
  ["missing uid", { uid: "" }],
] as const) {
  Deno.test(`rejects ${name}`, async () => {
    const t = await token(overrides);
    await assertRejects(async () => {
      await verifyTestGeneratorJwt(`Bearer ${t}`);
    });
  });
}
...
Deno.test("rejects malformed and expired tokens", async () => {
  await assertRejects(async () => {
    await verifyTestGeneratorJwt("Bearer malformed");
  });
  const expired = await new SignJWT({ uid: "student", feature: "test_generator", request_id: "request" })
    .setProtectedHeader({ alg: "RS256", kid: "test-kid" })
    ...
    .sign(privateKey);
  await assertRejects(async () => {
    await verifyTestGeneratorJwt(`Bearer ${expired}`);
  });
});
...
Deno.test("rejects a token signed by another private key", async () => {
  ...
  await assertRejects(async () => {
    await verifyTestGeneratorJwt(`Bearer ${forgedToken}`);
  });
});
