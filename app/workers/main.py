...
def register_handler(
    job_type: str, handler_func: Callable[[Dict[str, Any], asyncpg.Connection], Any]
):
...
class Worker:
        ...
        try:
            while self.running:
                async with pool.acquire() as conn:
                    ...
                    job = await service.lease_next_job(
                        self.worker_id, self.supported_types
                    )

                    if job:
                        logger.info(
                            f"Worker '{self.worker_id}' leased job '{job['id']}' of type '{job['job_type']}'"
                        )
                        self.active_task = asyncio.create_task(
                            self._process_job(pool, job)
                        )
    ...
    async def shutdown(self):
        logger.info(
            f"Worker '{self.worker_id}' received shutdown signal. Stopping job leasing..."
        )
        self.running = False
        ...
        while self.running:
            try:
                ...
                async with pool.acquire() as conn:
                    ...
                    if recovered > 0:
                        logger.warning(
                            f"Recovered {recovered} stale job(s) in worker background loop."
                        )
    ...
    async def _process_job(self, pool: asyncpg.Pool, job: Dict[str, Any]):
        ...
        if job_type not in HANDLERS:
            logger.error(
                f"Unsupported job type '{job_type}' for job '{job_id}'. Marking terminally failed."
            )
            async with pool.acquire() as conn:
                service = JobQueueService(conn)
            ...
            while heartbeat_running and self.running:
                ...
                try:
                    async with pool.acquire() as conn:
                        ...
                        if not hb_ok:
                            logger.warning(
                                f"Heartbeat failed for job '{job_id}'; worker may have lost lease ownership."
                            )
                except Exception as hb_err:
                    logger.error(
                        f"Error sending heartbeat for job '{job_id}': {hb_err}"
                    )
        ...
        try:
            async with pool.acquire() as conn:
                ...
                await service.update_progress(
                    job_id, self.worker_id, "processing", 10.0
                )
            ...
            async with pool.acquire() as conn:
                ...
                await service.fail_job(
                    job_id=job_id,
                    worker_id=self.worker_id,
                    error_code=getattr(err, "code", "HANDLER_ERROR"),
