...
async def embed_questions(
    ...
    candidates = []
    for row in await repo.fetch_questions_for_embedding(corpus_version_id, configuration_fingerprint):
        if row["language"] != "en":
            await repo.mark_embedding_failed(
                "chunk_expected_questions", str(row["id"]), "ENGLISH_ONLY_EMBEDDING_UNSUPPORTED"
            )
    ...
    for start in range(0, len(candidates), batch_size):
        ...
        try:
            ...
            if len(vectors) != len(batch):
                raise ValueError("Embedding provider returned an unexpected question vector count.")
            ...
            for row, _, _ in batch:
                await repo.mark_embedding_failed(
                    "chunk_expected_questions", str(row["id"]), "QUESTION_EMBEDDING_FAILED"
                )
            raise
...
async def handle_embed_questions(job: Dict[str, Any], conn: asyncpg.Connection) -> Dict[str, Any]:
