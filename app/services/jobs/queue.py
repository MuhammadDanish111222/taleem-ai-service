...
class JobQueueService:
    ...
    async def enqueue_job(
        ...
        return await self.repo.create_job(
            job_type, payload, idempotency_key, max_attempts
        )
    ...
    async def lease_next_job(
        ...
        return await self.repo.lease_job(
            worker_id, supported_types, lease_duration_seconds
        )
    ...
    async def update_progress(
        self, job_id: str, worker_id: str, stage: str, progress: float
    ) -> bool:
        return await self.repo.update_progress(job_id, worker_id, stage, progress)

    async def complete_job(
        self, job_id: str, worker_id: str, final_progress: float = 100.0
    ) -> bool:
    ...
    async def fail_job(
        ...
        return await self.repo.fail_job(
            job_id, worker_id, error_code, error_message, retry_delay_seconds
        )
